...
from saas.models import MagicLink, User, query
...
class State(rx.State):
    ...
    redirect_timer_amt: int = 4
    _redirect_route: str = ROUTES.DASHBOARD
    _redirect_task: asyncio.Task = None  # this doesnt work afaict
    ...
    def show_admin_info(self):
        with rx.session() as session:
            user = session.exec(User.select().where(User.is_admin.is_(True))).first()
    ...
    async def redirect_timer(self):
        """
        WARNING: Avoid redirects until it works with something like create_task which is cancellable
        ...
        dec_amt = 1
        time_left = self.redirect_timer_amt

        while time_left > 0:
            if self.redirect_cancel:
                console.log(f"redirect cancelled @ {time_left=}")
                async with self:
    ...
    @rx.event
    def toggle_cancel_redirect(self):
...
class AuthState(State):
    ...
    async def handle_login(self, data: dict = None):
        ...
        user = query.get_user_from_email(email=email)
    ...
    def on_load_verify(self):
        ...
        email, token, redir = params.get("email"), params.get("token"), params.get("redir")
        ...
        if (magic_link := query.check_magic_link(email=email, token=token)) is not None:
            ...
            self.session_token = magic_link.get_session_token()
            ...
            return rx.redirect(redir or ROUTES.DASHBOARD)
